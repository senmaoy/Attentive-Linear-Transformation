...
function LSTM.lstm(lstm_size,vocab_size)
  ...
  in_gate = nn.Linear(att_hid_size,r*feat_size)(in_gate)
  ...
  out_gate = nn.Linear(att_hid_size,r*rnn_size)(out_gate)
  out_gate = nn.Sigmoid()(out_gate)  
  
  local in_gater = nn.View(-1,r, feat_size)(in_gate)

  local out_gater = nn.View(-1, rnn_size)(out_gate)

  local img_exp = nn.Replicate(r,2)(img)
  local img_in =  nn.CMulTable(){img_exp,in_gater}
  img_in = nn.View(-1,feat_size)(img_in)
  local img_w = nn.Linear(feat_size,rnn_size)(img_in)
  local img_out =  nn.CMulTable(){out_gater,img_w}
  img_out  = nn.View(-1,r,rnn_size)(img_out)
  img_out = nn.Sum(2,3,true,true)(img_out)
