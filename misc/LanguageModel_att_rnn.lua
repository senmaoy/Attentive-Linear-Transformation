...
local LSTM = require 'misc.RNN_att'
...
function layer:__init(opt)
  ...
  self.core = LSTM.lstm( self.rnn_size, self.vocab_size + 1)
  self.lookup_table = nn.LookupTable(self.vocab_size + 1, self.input_encoding_size)
  self.linear = nn.Sequential()
  self.linear:add(nn.Linear(4096,512))
  self.linear:add(nn.ReLU(true))
...
function layer:getModulesList()
  return {self.core, self.linear,self.lookup_table}
end
...
function layer:parameters()
  ...
  local p3,g3 = self.linear:parameters()
  local params = {}
...
--imgs:fill(1)
    ...
    --table.insert(gate1, self.core.modules[11].output:clone())
    --table.insert(gate2, self.core.modules[17].output:clone())
   -- print(self.core.modules[11].output[1][200])
    --print(self.core.modules[17].output[1][1000])
    logprobs = out[self.num_state+1] -- last element is the output vector
    state = {}
    for i=1,self.num_state do table.insert(state, out[i]) end
  end
--print(self.core.modules[19].weight:sum()+self.core.modules[19].bias:sum(),self.core.modules[19].output:sum())
  -- return the samples and their log likelihoods
  return seq, seqLogprobs--,gate1,gate2,self.core.modules[19].weight
end
...
function layer:updateOutput(input)
  ...
  for t=1,self.seq_length+2 do
    ...
    if t == 1 then
      -- feed in the images
      xt = self.linear:forward(imgs) -- NxK sized inputz
    elseif t == 2 then
      -- feed in the start tokens
    ...
    if not can_skip then
      ...
      if t == 1 then 
        self.inputs[t] = {xt,imgs:clone():zero(),unpack(self.state[t-1])}      
      else
...
function layer:updateGradInput(input, gradOutput)
  local dimgs = input[1]:clone():zero() -- grad on input images
  ...
  for t=self.tmax,1,-1 do
    ...
    dimgs = dimgs + dinputs[2]
    -- continue backprop of xt
    if t == 1 then
      dimgs = dimgs + self.linear:backward(input[1],dxt)        
    else    
