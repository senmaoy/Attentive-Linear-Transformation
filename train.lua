...
require 'misc.LanguageModel_att'
local net_utils = require 'misc.net_utils'
require 'misc.optim_updates'
-------------------------------------------------------------------------------
-- Input arguments and options
-------------------------------------------------------------------------------
local a = utils.read_json('/home/yesenmao/application/eclipse/workspace/orient/checkpoint/map512.json')
require 'loadcaffe'
cmd = torch.CmdLine()
cmd:text()
...
-- Data input settings
cmd:option('-input_h5','/home/yesenmao/python scripts/coco_data_spilt.h5','path to the h5file containing the preprocessed dataset')
cmd:option('-input_json','/home/yesenmao/python scripts/coco_data_spilt.json','path to the json file containing additional info and vocab')
cmd:option('-cnn_proto','/home/yesenmao/application/eclipse/workspace/orient/model/VGG_ILSVRC_16_layers_deploy.txt','path to CNN prototxt file in Caffe format. Note this MUST be a VGGNet-16 right now.')
...
cmd:option('-start_from', '','./checkpoint/map_rnn_No_head.t7')
...
cmd:option('-val_images_use',4900, 'how many images to use when periodically evaluating the validation loss? (-1 = all)')
...
cmd:option('-id', 'r30', 'an id identifying this run/job. used in cross-val and appended when writing progress files')
...
--local res = torch.load('/home/yesenmao/dataset/resnet-152.t7')
-------------------------------------------------------------------------------
...
thin_lm.linear:share(protos.lm.linear, 'weight', 'bias')
...
while true do  
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    local checkpoint_path = './checkpoint/'..opt.id
