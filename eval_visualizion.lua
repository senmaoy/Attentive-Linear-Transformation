...
LSTM = require 'misc.RNN_att'
require 'misc.LanguageModel_att_rnn'
local net_utils = require 'misc.net_utils'
require 'image'
 --[[local core = LSTM.lstm{feat_size_att = 2,att_size = 4,lstm_size = 2,fc7_size = 2,att_hid_size = 2,vocab_size = 2}
local xt = torch.Tensor(2,2)
local xc = torch.Tensor(2,3)
local att_seq = torch.Tensor(2,4,2)
core:forward({xt,xt,xc})]]
--[[local core = LSTM.lstm(3,4)
local xt = torch.Tensor(2,3)
local xc = torch.Tensor(2,2)
core:forward({xc,xt,xc})]]
-------------------------------------------------------------------------------
...
-- Input paths
--cmd:option('-model','./checkpoint/m1.t7','path to model to evaluate')
cmd:option('-model','./checkpoint/map_lstm.t7','path to model to evaluate')
-- Basic options
cmd:option('-batch_size', 1, 'if > 0 then overrule, otherwise load from checkpoint.')
cmd:option('-num_images', 40, 'how many images to use when periodically evaluating the loss? (-1 = all)')
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    local seq, seqLogprobs,gate1,gate2,w = protos.lm:sample(feats, sample_opts)
    local weight = w:clone()
    weight= weight:view(512,512)
    weight=weight:t()
    local m = weight:max()
    local min = weight:min()
    local interval =m-min
    weight = (weight-min)/interval
    --print(weight)
    for g = 1,#gate1 do
      local p = torch.mm(gate2[g]:view(512,1), gate1[g]:view(1,512))
      p = torch.cmul(p,weight)
     print(gate2[g]:sum()/512,gate1[g]:sum()/512)
     --p= image.scale(p, 256, 32, 'bicubic')
     image.save('./vis/map/w' .. n .. '-' .. g .. '.png', w)
    end
    ...
    for k=1,#sents do
      ...
      if opt.dump_images == 1 then
       ...
       -- local cmd = 'cp "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" vis/imgs/img' .. #predictions .. '.jpg' -- bit gross
        local cmd = 'cp "' ..opt.image_root..''..data.infos[k].file_path..'' .. '" vis/imgs/img' .. #predictions .. '.jpg' -- bit gross
